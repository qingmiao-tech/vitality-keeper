(() => {
  ...
  function t(key, language = DEFAULT_LANGUAGE, params = {}) {
    ...
    return interpolate(messages[key] || fallback, params)
  }

  function applyI18n(language, root = document) {
    const normalized = normalizeLanguage(language)
    document.documentElement.lang = normalized

    root.querySelectorAll('[data-i18n]').forEach((element) => {
      element.textContent = t(element.getAttribute('data-i18n'), normalized)
    })

    root.querySelectorAll('[data-i18n-title]').forEach((element) => {
      element.setAttribute('title', t(element.getAttribute('data-i18n-title'), normalized))
    })

    root.querySelectorAll('[data-i18n-placeholder]').forEach((element) => {
      element.setAttribute('placeholder', t(element.getAttribute('data-i18n-placeholder'), normalized))
    })

    root.querySelectorAll('[data-i18n-aria-label]').forEach((element) => {
      element.setAttribute('aria-label', t(element.getAttribute('data-i18n-aria-label'), normalized))
    })
